<template>
    ...
    <ul class="flex handicap ft14">
      <li :class="{hand_active:is_active == i}" v-for="(item,i) in tabItem" :key="i" @click="is_active = i">{{item}}</li>
        ...
        <div class="content bg-part flex1 pt10" v-if="is_active == 0">
          <!--最新价-->
        ...
        <!--全站交易-->
        <div class="detail clr-part bg-part flex1 pt10"  v-if="is_active == 1">
...
export default {
  ...
  mounted(){
    ...
    eventBus.$on("toExchange0", function(data0) {
      //console.log('22222222222222',data0)
...
.content{padding: 10px 10px;height: 100%;}
...
.list-item li{line-height: 40px; display: flex;}

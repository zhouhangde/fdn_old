<template>
    <div class="indexBlackes mt45">
      ...
      <ul class="flex alcenter market_list">
        <li class="flex flex1 market_li around curPer" @click="goTrade(item.legal_id,item.currency_id,item.legal_name,item.currency_name,item.now_price,item.change,item.volume,item.cny_price)" v-for="(item,index) in sortList" :key="index" v-if="index<6">
          ...
          <ul class="list-con scroll" v-for="(item,index) in quotation" :key="index">
            ...
            <!-- <div v-if="search(li.currency_name)&&testItem(li.legal_name,li.added)"> -->
              ...
              <div class=""  style="color:#194B64;">
                <span class=" fr el-icon-star-on" v-if="li.added" @click="addDelete('user_delete',li.thisid)" style="line-height:30px;margin-left:20px"></span>
                <span class=" fr el-icon-star-off" v-if="!li.added" style="line-height:30px;margin-left:20px" @click="addDelete('user_add',li.thisid)"></span>

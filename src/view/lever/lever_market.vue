		...
		<ul class="coin-wrap scroll">
			<li v-for="(market,index) in marketList " :key="index">
				<p v-for="(itm,idx) in market" :key="itm.id"
				   v-show="search(itm.currency_name)&&testItem(itm.legal_name,itm.added)"
	...
	export default {
		...
		methods: {
			...
			quota_shift(
				...
				eventBus.$emit('upDatePk');
				// 存本地
				window.localStorage.setItem("leverTradeData_my", JSON.stringify(leverTradeDatas));

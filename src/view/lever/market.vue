	...
	export default {
			...
			this.$http({
				...
				.then(res => {
					if (res.data.type == "ok") {
						...
						that.marketList = res.data.message[index].array;
						//console.log(that.marketList);
						//console.log(res.data.message[index].array)
						that.selectedId =res.data.message[index].array[that.index2].legal_id;
						that.dataList = res.data.message;
						that.shareNum = res.data.message[index].array[that.index2].lever_share_num;
						window.localStorage.setItem("shareNum", that.shareNum);
						//默认法币id和name
						//console.log(localStorage.getItem("legal_id"))
						if (!localStorage.getItem("legal_id") &&!localStorage.getItem("currency_id") &&!localStorage.getItem("legal_name") &&!localStorage.getItem("currency_name")) {
							//console.log(msg,indexs,'99999999999999999999999999999999999999')
							that.currency_name = msg[index].array[indexs].currency_name;
							that.currency_id = msg[index].array[indexs].currency_id;
							window.localStorage.setItem(
								"currency_name",
								msg[index].array[indexs].currency_name
							);
							window.localStorage.setItem(
								"legal_id",
								msg[index].array[indexs].legal_id
							);
							window.localStorage.setItem(
								"currency_id",
								msg[index].array[indexs].currency_id
							);
							window.localStorage.setItem(
								"lever_currency_name",
								msg[index].array[indexs].currency_name
		...
		methods: {
			changeType(index, currency, currency_id) {
				...
				this.marketList = this.dataList[index].array;
			},
			getSymbols(callback) {
			...
			socket() {
				...
				ws.onmessage = function (evt){
					...
					if (msg.type == "daymarket") {
						//console.log(msg)
						$.each(that.marketList,function(k,v){
							if(v.legal_id == msg.legal_id && v.currency_id == msg.currency_id){

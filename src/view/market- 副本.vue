    ...
    <div class="market clr-part">
        ...
        <div class="m_filter">
            <div class="tabtitle ft14 curPer flex around">
                <span v-for="(tab,index) in tabList" :key="index" :class="['bdr-part',{'white_active':((index == isShow)&&!showAdd)}]" @click="changeType(index,tab.name,tab.area_id)">{{tab.name}}</span>
                <span @click="showAdd=true" :class="['bdr-part',{'white_active': showAdd}]">{{$t('home.myMarkets')}}</span>
            </div>
        </div>
        <div v-if="!issearch">
        <div v-if="!showAdd">
        ...
        <ul class="coin-wrap scroll">
          <li v-for="(market,index) in tabList " :key="index" v-if="index==areaIndex">
            <div v-for="(itms,idx) in market.data"  v-if="idx == legalIndex">
              ...
              <span class="w36"><img :src="itm.logo" alt="" v-if="false"><i><em class="deep_blue bold blue_clr">{{itm.currency_name}}</em><em class="light_blue bold blue_clr">/{{itm.legal_name}}</em></i></span>
              <span class="w30 tr deep_blue blue_clr bold nowPrice blue_clr" :data-name='itm.currency_id+"/"+itm.legal_id'>{{itm.now_price || 0}}</span>
            ...
            <li>
              <div>
                 <p v-for="(itm,j) in myAdd " :key="j" class="ft12" style="line-height:30px;" :class="{'bg-even':j%2 !=0}">
                   <span class="w30 inblock"><i><em class="deep_blue blue_clr bold">{{itm.currency_name}}</em><em class="light_blue bold blue_clr">/{{itm.legal_name}}</em></i></span>
                   <span class="w30 inblock tc deep_blue blue_clr bold nowPrice">{{itm.now_price || 0}}</span>
